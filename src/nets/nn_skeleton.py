    ...
    with tf.variable_scope('bbox_pred') as scope:
        with tf.variable_scope('stretching'):
            ...
            box_width = tf.identity(
                ...
                name='bbox_width') # exponent is used to undo the log used to pack box width and height
            box_height = tf.identity(
                anchor_h * util.safe_exp(delta_h, mc.EXP_THRESH),
                name='bbox_height') # exponent is used to undo the log used to pack box width and height
